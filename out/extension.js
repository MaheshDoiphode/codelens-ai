...
const minimatch_1 = require("minimatch"); // <-- Import minimatch
class SessionFileStorage {
    _files = new Map();
    sessionId;
    constructor(sessionId) { this.sessionId = sessionId; }
    get files() { return Array.from(this._files.values()); }
    get filesOnly() { return this.files.filter(f => !f.isDirectory).map(f => ({ path: f.path, content: f.content })); }
    async addFile(filePath, parentPath) { const n = path.normalize(filePath); if (this._files.has(n))
        return false; let c = null; try {
        c = await fs.readFile(n, 'utf8');
    }
    catch (e) {
        vscode.window.showErrorMessage(`Read file error: ${e.message}`);
        console.error(`Read file error ${n}:`, e);
    } const f = { path: n, content: c, isDirectory: false, parent: parentPath ? path.normalize(parentPath) : undefined, sessionId: this.sessionId }; this._files.set(n, f); return true; }
    async addDirectory(dirPath, parentPath) { const n = path.normalize(dirPath); if (this._files.has(n) && this._files.get(n)?.isDirectory)
        return false; const d = { path: n, content: null, isDirectory: true, parent: parentPath ? path.normalize(parentPath) : undefined, sessionId: this.sessionId }; this._files.set(n, d); try {
        const es = await fs.readdir(n, { withFileTypes: true });
        const ps = [];
        for (const e of es) {
            const fp = path.join(n, e.name);
            if (e.isDirectory())
                ps.push(this.addDirectory(fp, n));
            else if (e.isFile())
                ps.push(this.addFile(fp, n));
        }
        await Promise.all(ps);
    }
    catch (e) {
        vscode.window.showErrorMessage(`Read dir error: ${e.message}`);
        console.error(`Read dir error ${n}:`, e);
        return false;
    } return true; }
    removeEntry(entryPath) { const n = path.normalize(entryPath); const e = this._files.get(n); if (!e)
        return false; let rc = 0; if (this._files.delete(n))
        rc++; if (e.isDirectory) {
        const p = n + path.sep;
        const k = Array.from(this._files.keys());
        k.forEach(key => { if (key.startsWith(p) && this._files.delete(key))
            rc++; });
    } return rc > 0; }
    clearFiles() { const c = this._files.size; this._files.clear(); return c; }
}
class Session {
    id;
    name;
...
class FileIntegratorProvider {
        ...
        await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Adding files to session "${targetSession.name}"...`, cancellable: true }, async (progress, progressToken) => {
            ...
            for (let i = 0; i < uris.length; i++) {
                ...
                try {
                    ...
                    if (!processed) {
                        skippedCount++;
                    } // Increment if processPath indicated skip
                }
                catch (err) {
    ...
    async processPath(filePath, session, token) {
        ...
        try {
            ...
            if (!exists)
                return true; // Indicate processed (but didn't add)
            if (token.isCancellationRequested)
                return false;
...
function generateMarkdownContent(session) {
    let c = '';
    const f = session.storage.filesOnly.sort((a, b) => a.path.localeCompare(b.path));
    if (f.length === 0)
