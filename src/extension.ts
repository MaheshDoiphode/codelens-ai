...
import { minimatch } from 'minimatch'; // For exclusions
...
class SessionFileStorage {
    ...
    removeEntry(entryPath: string): boolean {
        ...
        if (mainIndex === -1) return false; // Not found
        ...
        this._files = this._files.filter(f => {
            if (f.path === normalizedPath) return false; // Remove main entry
            if (prefix && f.path.startsWith(prefix)) return false; // Remove descendants
    ...
    reorderItems(draggedPaths: string[], targetPath?: string, dropOnSession: boolean = false): boolean {
        ...
        if (draggedEntries.length === 0) return false; // Nothing to reorder
        ...
        if (dropOnSession) {
            ...
            if (targetIndex === -1) targetIndex = this._files.length; // Append if no root items exist
...
class FileIntegratorProvider implements vscode.TreeDataProvider<IntegratorTreeItem>, vscode.TreeDragAndDropController<IntegratorTreeItem> {
    ...
    async handleDrop(target: IntegratorTreeItem | undefined, dataTransfer: vscode.DataTransfer, token: vscode.CancellationToken): Promise<void> {
        ...
        if (internalDropItem) {
            ...
            const draggedItemIds = internalDropItem.value as string[]; // We stored array of strings (TreeItem IDs)
    ...
    private async processPath(filePath: string, session: Session, token: vscode.CancellationToken): Promise<boolean> {
        ...
        if (isPathExcluded(filePath)) { console.log(`[Exclude] Skipping excluded path: ${filePath}`); return false; }
        try {
             const exists = await fs.pathExists(filePath); if (!exists) return true;
             if (token.isCancellationRequested) return false;
            const stats = await fs.stat(filePath); if (token.isCancellationRequested) return false;
             ...
             return true;
        } catch (err: any) { vscode.window.showErrorMessage(`Error processing path ${path.basename(filePath)}: ${err.message}`); console.error(`Error processing path ${filePath}:`, err); return true; }
    }
}
...
function isPathExcluded(filePath: string): boolean { /* ... (No changes needed) ... */
    const config = vscode.workspace.getConfiguration('fileintegrator'); const excludePatterns = config.get<Record<string, boolean>>('exclude'); if (!excludePatterns) return false;
    const workspaceFolders = vscode.workspace.workspaceFolders; const normalizedFilePath = filePath.replace(/\\/g, '/');
    for (const pattern in excludePatterns) { if (excludePatterns[pattern]) { const normalizedPattern = pattern.replace(/\\/g, '/'); if (minimatch(normalizedFilePath, normalizedPattern, { dot: true })) return true; if (workspaceFolders) { for (const folder of workspaceFolders) { const folderPath = folder.uri.fsPath.replace(/\\/g, '/'); if (normalizedFilePath.startsWith(folderPath + '/')) { const relativePath = normalizedFilePath.substring(folderPath.length + 1); if (minimatch(relativePath, normalizedPattern, { dot: true })) return true; } } } if (minimatch(path.basename(normalizedFilePath), normalizedPattern, { dot: true, matchBase: true })) return true; } } return false;
 }
 ...
 */
 function generateMarkdownContent(session: Session): string {
    let content = '';
